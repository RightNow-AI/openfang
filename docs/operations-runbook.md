...
The shipped systemd unit now performs a fail-closed pre-start check. It always verifies that the binary exists, `config.toml` is readable, and the runtime home/data directories are writable. In strict production mode, `/usr/local/lib/openfang/preflight-openfang.sh` is treated as mandatory: if the helper is missing, the unit refuses to start rather than silently skipping the deeper config/state validation. When the helper is present, the unit runs `preflight-openfang.sh --offline` before every start so malformed config or broken state files fail fast instead of turning into a restart loop.
...
Use `OPENFANG_STRICT_PRODUCTION=1` in deployment automation so missing machine auth becomes a hard failure instead of a warning. The shipped systemd unit now enables this by default in its pre-start gate so host installs fail closed the same way the Docker/Compose path does.
...
- the metrics set now includes `openfang_readiness_ready`, `openfang_database_ok`, `openfang_usage_store_ok`, `openfang_shutdown_requested`, `openfang_default_provider_auth_missing`, `openfang_config_warnings`, and `openfang_restore_warnings`
...
`BACKUP.txt` now also records `openfang_binary`, `openfang_version`, `openfang_binary_sha256`, and `openfang_git_sha`, so rollback runs know exactly which binary (and build) the snapshot was created for.
