...
`scripts/smoke-openfang.sh`, `scripts/live-api-smoke-openfang.sh`, and `scripts/provider-canary-openfang.sh` now also honor `OPENFANG_ENV_FILE` for machine auth and listen-address resolution, so systemd hosts can reuse `/etc/openfang/env` instead of re-exporting `OPENFANG_API_KEY` by hand. `scripts/smoke-openfang.sh` also validates the public dashboard shell (`/`) and confirms `/api/metrics` still exposes the operational metric families used by the bundled alerts and runbooks, so a broken SPA bundle or alert drift fails during smoke instead of after cutover.
...
Add `scripts/live-api-smoke-openfang.sh` to your post-cutover checklist so the agent spawn/budget/kill workflow runs against each target environment before you declare it ready. On systemd hosts, `OPENFANG_ENV_FILE=/etc/openfang/env` is enough for auth/base-url resolution unless you intentionally want to override them.
