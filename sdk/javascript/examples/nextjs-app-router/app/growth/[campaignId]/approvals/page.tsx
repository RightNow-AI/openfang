...
const APPROVAL_LABELS: Record<string, string> = {
  draft_approval: "Draft Review", send_approval: "Send",
  tool_use_approval: "Tool Use", publish_approval: "Publish",
  client_delivery_approval: "Client Delivery",
  student_facing_content_approval: "Student Content",
  spend_approval: "Spend",
};

export default function GrowthApprovalsPage({ params }: Props) {
  const [campaignId, setCampaignId] = useState("");
  const [approvals, setApprovals]   = useState<ModeApproval[]>([]);
  const [error, setError]           = useState("");

  const reload = (cid: string) =>
    getModeApprovals("growth", cid)
      .then((r) => setApprovals(r.approvals))
      .catch((e: Error) => setError(e.message));

  useEffect(() => {
    params.then(({ campaignId: cid }) => { setCampaignId(cid); reload(cid); });
  }, [params]);

  async function handleApprove(taskId: string) {
    await approveModeTask("growth", taskId);
    reload(campaignId);
  }
  ...
  return (
    <main style={{ padding: "24px 32px", maxWidth: 860, margin: "0 auto" }}>
      ...
      <h1 style={{ fontSize: 22, fontWeight: 700, marginTop: 8, marginBottom: 4 }}>Approvals</h1>
