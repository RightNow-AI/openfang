...
export default function CommsDraftDrawer({ open, draftId, onClose, onApprove, onRequestChanges, onSend }) {
  ...
  return (
    ...
    >
      <div data-cy="comms-draft-drawer" style={{ width: 520, background: 'var(--bg-elevated,#111)', borderLeft: '1px solid var(--border,#333)', overflowY: 'auto', padding: '28px 24px', display: 'flex', flexDirection: 'column', gap: 0 }}>
        ...
        {draft && (
          <>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              {draft.channel && <span style={{ fontSize: 11, color: 'var(--text-dim,#888)' }}>via {draft.channel}</span>}
            </div>

            {/* Recipients */}
            {draft.recipients?.length > 0 && (
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-dim,#888)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 5 }}>Recipients</div>
            ...
            <div style={{ marginBottom: 18 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-dim,#888)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Message</div>
              <div style={{ fontSize: 14, lineHeight: 1.7, color: 'var(--text-primary,#f1f1f1)', whiteSpace: 'pre-wrap', background: 'var(--surface2,#1a1a2e)', padding: '12px 14px', borderRadius: 8, border: '1px solid var(--border,#333)' }}>
                {draft.body}
              </div>
            </div>
            ...
            {showChangeInput && (
              <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                <input
                  ...
                  style={{ flex: 1, padding: '8px 12px', borderRadius: 7, background: 'var(--bg-elevated,#111)', border: '1px solid var(--border,#333)', color: 'var(--text-primary,#f1f1f1)', fontSize: 13, outline: 'none' }}
                />
                <button
                  onClick={() => act(() => onRequestChanges?.(draftId, changeNote), 'change')}
                  disabled={!changeNote.trim() || working === 'change'}
                  style={{ padding: '8px 14px', borderRadius: 7, background: 'transparent', border: '1px solid var(--border,#333)', color: 'var(--text-dim,#888)', cursor: 'pointer', fontSize: 13 }}
                >
                  {working === 'change' ? '…' : 'Send'}
                </button>
              </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 'auto', paddingTop: 20, borderTop: '1px solid var(--border,#333)', flexWrap: 'wrap' }}>
              ...
              {draft.approval_status === 'approved' && (
                <button onClick={() => act(() => onSend?.(draftId), 'send')} disabled={!!working} style={{ padding: '9px 20px', borderRadius: 8, background: 'var(--accent,#7c3aed)', color: '#fff', border: 'none', cursor: working ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: 14, opacity: working && working !== 'send' ? 0.5 : 1 }}>
                  {working === 'send' ? 'Sending…' : 'Send'}
                </button>
              )}
              <button onClick={() => setShowChangeInput(v => !v)} style={{ padding: '9px 14px', borderRadius: 8, background: 'transparent', border: '1px solid var(--border,#333)', color: 'var(--text-dim,#888)', cursor: 'pointer', fontSize: 13 }}>Request changes</button>
              <button onClick={onClose} style={{ padding: '9px 14px', borderRadius: 8, background: 'transparent', border: '1px solid var(--border,#333)', color: 'var(--text-dim,#888)', cursor: 'pointer', fontSize: 13 }}>Close</button>
            </div>
          </>
        )}
      </div>
