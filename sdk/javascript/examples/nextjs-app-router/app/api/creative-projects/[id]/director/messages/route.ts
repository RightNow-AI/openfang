...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  try {
    const res = await fetch(`${BASE}/api/creative-projects/${id}/director/messages`, { cache: 'no-store' });
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  try {
    const res = await fetch(`${BASE}/api/creative-projects/${id}/director/messages`, {
      method: 'POST',
