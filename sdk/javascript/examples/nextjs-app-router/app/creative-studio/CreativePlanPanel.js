  ...
  return (
    <div data-cy="plan-panel" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>Creative thesis</div>
      ...
      {plan.status !== 'approved' && (
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <button
            ...
            style={{ padding: '9px 16px', borderRadius: 8, background: 'transparent', border: '1px solid var(--border)', color: 'var(--text-dim)', cursor: 'pointer', fontSize: 13 }}
          >
            Request revision
          </button>
        </div>
      ...
      {showRevise && (
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ...
            style={{ flex: 1, padding: '9px 12px', borderRadius: 7, background: 'var(--bg-elevated)', border: '1px solid var(--border)', color: 'var(--text-primary)', fontSize: 13, outline: 'none' }}
          ...
          <button
            ...
            style={{ padding: '9px 16px', borderRadius: 7, background: 'var(--accent)', color: '#fff', border: 'none', cursor: reviseNote.trim() ? 'pointer' : 'not-allowed', fontWeight: 600, fontSize: 13, opacity: reviseNote.trim() ? 1 : 0.5 }}
          >
            Send
          </button>
        </div>
        ...
        <div style={{ borderTop: '1px solid var(--border)', paddingTop: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {Object.keys(TASK_LABELS).map(t => (
              <button key={t} onClick={() => onLaunchTask(t)} style={{ padding: '7px 13px', borderRadius: 7, background: 'transparent', border: '1px solid var(--border)', color: 'var(--text-primary)', cursor: 'pointer', fontSize: 12 }}>
                {TASK_LABELS[t]}
              </button>
            ))}
          </div>
...
function PlanSection({ title, content, items }) {
  return (
    <div style={{ padding: '12px 14px', borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)' }}>
      <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 }}>{title}</div>
