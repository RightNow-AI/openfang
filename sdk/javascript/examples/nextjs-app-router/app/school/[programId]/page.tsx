  ...
  return (
    <div style={{ padding: 32, maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ height: 7, background: "var(--border, #333)", borderRadius: 999, overflow: "hidden" }}>
          <div style={{ height: "100%", background: "var(--accent, #7c3aed)", borderRadius: 999, width: `${pct}%`, transition: "width 0.4s" }} />
        </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 28, flexWrap: "wrap" }}>
        {hasHealthTracking && (
          <Link href={`/school/${programId}/cohort`}>
            <button style={{ padding: "8px 16px", borderRadius: 6, background: "var(--accent, #7c3aed)", color: "#fff", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
              🎓 Cohort Health
            </button>
          </Link>
        )}
        <Link href={`/school/${programId}/approvals`}>
          <button style={{ padding: "8px 16px", borderRadius: 6, background: "transparent", color: "var(--accent, #7c3aed)", border: "1px solid var(--accent, #7c3aed)", cursor: "pointer", fontSize: 13 }}>
            Approvals
          </button>
        </Link>
      ...
      {tasks.length === 0 ? (
        <div style={{ padding: 20, border: "1px dashed var(--border, #333)", borderRadius: 8, textAlign: "center", color: "var(--text-muted, #888)", fontSize: 14 }}>
          No tasks yet. Generate a plan to get started.
        </div>
      ) : (
        <div>
          {tasks.map((t) => (
            <div key={t.id} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 14px", border: "1px solid var(--border, #333)", borderRadius: 8, marginBottom: 8 }}>
