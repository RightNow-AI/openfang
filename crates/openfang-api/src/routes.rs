...
"#;
    ...
    #[test]
    fn test_resolve_session_for_attachments_accepts_fresh_agent_session() {
        let (kernel, _tmp) = boot_kernel();
        let agent_a = spawn_agent(&kernel, "agent-a");
        let session_a = kernel.registry.get(agent_a).unwrap().session_id;

        let resolved = resolve_session_for_attachments(&kernel, agent_a, Some(session_a)).unwrap();
        assert_eq!(resolved, session_a);
    }
