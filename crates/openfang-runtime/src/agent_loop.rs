...
pub async fn run_agent_loop(
    ...
    // Track the index so we can roll back if the LLM call fails entirely,
    // preventing orphaned user messages from accumulating and poisoning context.
    let user_msg_index = session.messages.len();
    ...
    for iteration in 0..max_iterations {
        ...
        let mut response = match call_with_retry(&*driver, request, Some(provider_name), None).await {
            Ok(r) => r,
            Err(e) => {
                // Roll back the user message so failed calls don't poison session history.
                // Without this, repeated LLM errors accumulate orphaned user messages that
                // bloat context and cause cascading failures on subsequent attempts.
                if session.messages.len() > user_msg_index {
                    warn!(
                        agent = %manifest.name,
                        rolled_back = session.messages.len() - user_msg_index,
                        "Rolling back session messages after LLM error"
                    );
                    session.messages.truncate(user_msg_index);
                }
                return Err(e);
            }
        };
            ...
            Err(e) => {
                ...
                if session.messages.len() > user_msg_index {
                    warn!(
                        agent = %manifest.name,
                        rolled_back = session.messages.len() - user_msg_index,
                        "Rolling back session messages after streaming LLM error"
                    );
                    session.messages.truncate(user_msg_index);
                }
                return Err(e);
