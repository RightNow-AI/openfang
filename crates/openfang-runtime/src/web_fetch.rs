...
impl WebFetchEngine {
    ...
    pub async fn fetch_with_options(
        ...
        // Step 5: Truncate (by char boundary, not byte)
        let truncated = if processed.chars().count() > self.config.max_chars {
            let truncated_content: String = processed.chars().take(self.config.max_chars).collect();
            format!(
