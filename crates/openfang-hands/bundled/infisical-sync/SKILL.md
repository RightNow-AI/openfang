...
**Endpoint**: `POST /api/v3/secrets/raw/{secretName}`

Acts as an upsert — creates if absent, updates if exists.
...
curl -s -X POST \
  ...
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"workspaceId\":\"$PROJECT_ID\",\"environment\":\"$ENVIRONMENT\",\"secretValue\":\"$SECRET_VALUE\",\"secretPath\":\"/\"}"
```
...
### Delete a Secret
**Endpoint**: `DELETE /api/v3/secrets/raw/{secretName}`

**Query parameters**: `workspaceId`, `environment`, `secretPath` (default `/`)

**curl example**:
```bash
curl -s -X DELETE \
  "$INFISICAL_URL/api/v3/secrets/raw/$SECRET_NAME?workspaceId=$PROJECT_ID&environment=$ENVIRONMENT" \
  -H "Authorization: Bearer $ACCESS_TOKEN"
```
