...
fn init_tracing_stderr() {
    ...
    if std::env::var("OPENFANG_LOG_FORMAT").as_deref() == Ok("json") {
...
fn main() {
    ...
    match cli.command {
        ...
        Some(Commands::Security(sub)) => match sub {
            ...
            SecurityCommands::HashPassword { password } => cmd_security_hash_password(password),
...
fn cmd_security_hash_password(password: Option<String>) {
    ...
    let password = if let Some(password) = password {
        ...
        if io::stdin().read_line(&mut input).is_err() {
            ui::error("Failed to read password from stdin.");
            std::process::exit(1);
        }
        input
    } else {
