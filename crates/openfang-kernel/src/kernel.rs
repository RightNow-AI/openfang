    ...
    pub fn start_background_agents(self: &Arc<Self>) {
        ...
        if !saved_hands.is_empty() {
            ...
            for saved in saved_hands {
                if self.hand_registry.get_definition(&saved.hand_id).is_none() {
                    let restore_definition = if let Some(source_path) = saved.source_path.as_ref() {
                        match self.hand_registry.install_from_path(source_path) {
                            Ok(def) => Ok(def),
                            Err(err) => {
                                warn!(
                                    hand = %saved.hand_id,
                                    ...
                                    error = %err,
                                    "Failed to reload external hand from source path; falling back to persisted content"
                                );
                                match saved.toml_content.as_deref() {
                                    Some(toml_content) => self.hand_registry.install_from_content(
                                        toml_content,
                                        saved.skill_content.as_deref().unwrap_or(""),
                                    ),
                                    None => Err(err),
                                }
                            }
                        }
                    } else if let Some(toml_content) = saved.toml_content.as_deref() {
                        self.hand_registry.install_from_content(
                            toml_content,
                            saved.skill_content.as_deref().unwrap_or(""),
                        )
                    } else {
                        Err(openfang_hands::HandError::NotFound(format!(
                            "No persisted source for hand '{}'",
                            saved.hand_id
                        )))
                    };

                    if let Err(err) = restore_definition {
                        warn!(
                            hand = %saved.hand_id,
                            error = %err,
                ...
                match self.activate_hand(&saved.hand_id, saved.config) {
                    Ok(inst) => {
